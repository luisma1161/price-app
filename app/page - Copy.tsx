...
// --- TYPES ---
// We keep the interface definition to prevent TS errors, even if unused for now
declare global {
  interface Window {
    electronAPI: {
      saveQuote: (data: any) => void;
      onSaveResponse: (callback: (arg: { success: boolean, path?: string, error?: string }) => void) => void;
      loadQuote: () => void;
      onLoadResponse: (callback: (arg: { success: boolean, data?: any, error?: string }) => void) => void;
      saveAndExport: (data: any) => void;
    };
  }
}
...
// Smart NumberField (Prevents locking while typing)
...
function TypeChooser({ value, onChange }: { value: BoxType; onChange: (t: BoxType) => void; }) {
  return (
    <div className="flex items-center gap-4">
      <label className="flex items-center gap-3 border rounded-xl px-4 py-2 cursor-pointer hover:bg-gray-50"><input type="checkbox" className="big-check" checked={value === 'cooler'} onChange={() => onChange('cooler')} aria-label="Set type to Cooler" tabIndex={-1} /><span className="text-base font-medium">Cooler</span></label>
      <label className="flex items-center gap-3 border rounded-xl px-4 py-2 cursor-pointer hover:bg-gray-50"><input type="checkbox" className="big-check" checked={value === 'freezer'} onChange={() => onChange('freezer')} aria-label="Set type to Freezer" tabIndex={-1} /><span className="text-base font-medium">Freezer</span></label>
    </div>
  );
}
...
export default function Page() {
  ...
  function handlePrint() {
    ...
    if (typeof window !== "undefined") {
      ...
      document.title = meta.quoteNo ? `Quote-${meta.quoteNo}` : "Quote";
      window.print();
    ...
    <>
      <style jsx global>{`
        ...
        @media print {
          ...
          .print-container { max-width: 100%; font-size: 12px; line-height: 1.4; }
          ...
          .print-table th, .print-table td { border: 1px solid #e5e5e5; padding: 6px 8px; text-align: left; }
      ...
      <main className="mx-auto max-w-7xl p-6 grid gap-6 print-scale">
        ...
        <section className="border rounded-2xl p-4 grid gap-4 no-print">
          <div className="flex items-center justify-between">
            ...
            <TypeChooser value={box1.type} onChange={(t) => setBox1({ ...box1, type: t })} />
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={box1.floorOn} onChange={(e) => setBox1({ ...box1, floorOn: e.target.checked })} />
              <span>Include Floor</span>
            </label>
            <label className="flex items-center gap-2">
              <span>Doors</span>
              <NumberField label="" value={box1.doors} onChange={(n) => setBox1({ ...box1, doors: n })} />
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={box1.deductOn} onChange={(e) => setBox1({ ...box1, deductOn: e.target.checked })} />
              <span>Deduct a custom wall</span>
            </label>
          </div>
          {box1.deductOn && (
        ...
        <section className="border rounded-2xl p-4 grid gap-4 no-print">
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2">
              <input ref={includeBox2Ref} type="checkbox" checked={includeBox2} onChange={(e) => toggleIncludeBox2(e.target.checked)} />
              <span>Include Box 2</span>
            </label>
            {includeBox2 && <TypeChooser value={box2.type} onChange={(t) => setBox2({ ...box2, type: t })} />}
          </div>
          {includeBox2 && (
            <>
              <div className="grid md:grid-cols-3 gap-4">
              ...
              <div className="grid md:grid-cols-3 gap-3">
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={box2.floorOn} onChange={(e) => setBox2({ ...box2, floorOn: e.target.checked })} />
                  <span>Include Floor</span>
                </label>
                <label className="flex items-center gap-2">
                  <span>Doors</span>
                  <NumberField label="" value={box2.doors} onChange={(n) => setBox2({ ...box2, doors: n })} />
                </label>
                <label className="flex items-center gap-2">
                  <input type="checkbox" checked={box2.deductOn} onChange={(e) => setBox2({ ...box2, deductOn: e.target.checked })} />
                  <span>Deduct a custom wall</span>
                </label>
              </div>
              {box2.deductOn && (
        ...
        <div className="print-only print-container">
          ...
          <div className="print-section">
            <div 
              ...
              style={{ 
                backgroundColor: box1.type === 'cooler' ? '#1e40af' : '#0e7490', 
                color: 'white' 
              }}
            >
              Box 1 — {box1.type.toUpperCase()}
            </div>
            <table className="print-table">
              <tbody>
                ...
                {box1.deductOn && (
                  <tr>
                    <td><strong>Deduct Wall</strong></td>
                    <td colSpan={3}>{feetInToText(box1.deductLenFt, box1.deductLenIn)} × {feetInToText(box1.deductHtFt, box1.deductHtIn)} = {calc1.deductedArea.toFixed(1)} ft²</td>
                  </tr>
                )}
                {box1.doors > 0 && (
                  <tr>
                    <td><strong>Doors</strong></td>
                    <td colSpan={3}>{box1.doors} × {currency(doorPrice1)} = <span className="bold">{currency((box1.doors || 0) * (doorPrice1 || 0))}</span></td>
                  </tr>
                )}
                {addOns1.length > 0 && (
                  <tr>
                    <td><strong>Add-ons</strong></td>
                    <td colSpan={3}>{addOns1.map((a, i) => <span key={i} style={{ marginRight: '15px' }}>{a.label}: {a.qty} × {currency(a.unit)} = {currency((a.qty || 0) * (a.unit || 0))}</span>)}</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div style={{ display: 'flex', borderTop: '1px solid #ccc' }}>
              <div style={{ flex: 1, borderRight: '1px solid #ccc', padding: '8px' }}>
                ...
                <div>Walls: {unit1.wall}</div>
                <div>Ceiling: {unit1.ceiling}</div>
                <div>Floor: {unit1.floor}</div>
              </div>
              <div style={{ flex: 1, borderRight: '1px solid #ccc', padding: '8px' }}>
                <div className="bold" style={{ marginBottom: '4px' }}>Areas (ft²)</div>
                <div>Walls: {calc1.wallsArea.toFixed(2)}</div>
                <div>Ceiling: {calc1.slabArea.toFixed(2)}</div>
                <div>Floor: {box1.floorOn ? calc1.slabArea.toFixed(2) : "0.00"}</div>
              </div>
              <div style={{ flex: 1, padding: '8px', backgroundColor: '#f9f9f9' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px', borderTop: '1px solid #ccc', paddingTop: '4px' }}>
                  <span className="bold">Box 1 Total:</span> <span className="bold">{currency(calc1.subtotal)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {includeBox2 && (
            <div className="print-section">
              <div 
                className="print-section-header"
                style={{ 
                  backgroundColor: box2.type === 'cooler' ? '#1e40af' : '#0e7490', 
                  color: 'white' 
                }}
              >
                Box 2 — {box2.type.toUpperCase()}
              </div>
              <table className="print-table">
                <tbody>
                  <tr>
                  ...
                  {box2.deductOn && (
                    <tr>
                      <td><strong>Deduct Wall</strong></td>
                      <td colSpan={3}>{feetInToText(box2.deductLenFt, box2.deductLenIn)} × {feetInToText(box2.deductHtFt, box2.deductHtIn)} = {calc2.deductedArea.toFixed(1)} ft²</td>
                    </tr>
                  )}
                  {box2.doors > 0 && (
                    <tr>
                      <td><strong>Doors</strong></td>
                      <td colSpan={3}>{box2.doors} × {currency(doorPrice2)} = <span className="bold">{currency((box2.doors || 0) * (doorPrice2 || 0))}</span></td>
                    </tr>
                  )}
                  {addOns2.length > 0 && (
                    <tr>
                      <td><strong>Add-ons</strong></td>
                      <td colSpan={3}>{addOns2.map((a, i) => <span key={i} style={{ marginRight: '15px' }}>{a.label}: {a.qty} × {currency(a.unit)} = {currency((a.qty || 0) * (a.unit || 0))}</span>)}</td>
                    </tr>
                  )}
                </tbody>
              </table>
              ...
              <div style={{ display: 'flex', borderTop: '1px solid #ccc' }}>
                <div style={{ flex: 1, borderRight: '1px solid #ccc', padding: '8px' }}>
                  <div className="bold" style={{ marginBottom: '4px' }}>Unit Prices ($/ft²)</div>
                  <div>Walls: {unit2.wall}</div>
                  <div>Ceiling: {unit2.ceiling}</div>
                  <div>Floor: {unit2.floor}</div>
                </div>
                <div style={{ flex: 1, borderRight: '1px solid #ccc', padding: '8px' }}>
                  <div className="bold" style={{ marginBottom: '4px' }}>Areas (ft²)</div>
                  <div>Walls: {calc2.wallsArea.toFixed(2)}</div>
                  <div>Ceiling: {calc2.slabArea.toFixed(2)}</div>
                  <div>Floor: {box2.floorOn ? calc2.slabArea.toFixed(2) : "0.00"}</div>
                </div>
                <div style={{ flex: 1, padding: '8px', backgroundColor: '#f9f9f9' }}>
                  <div className="bold" style={{ marginBottom: '4px' }}>Calculated Costs</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px', borderTop: '1px solid #ccc', paddingTop: '4px' }}>
                    <span className="bold">Box 2 Total:</span> <span className="bold">{currency(calc2.subtotal)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="print-section" style={{ breakInside: 'avoid' }}>
            <table className="print-table summary-table">
              <tbody>
                <tr>
                  <td className="text-right" style={{ width: '70%' }}>Subtotal (Boxes):</td>
                  <td className="text-right bold">{currency(subTotal)}</td>
                </tr>
                <tr>
                  <td className="text-right">Profit ({prices.profitPct}%):</td>
                  <td className="text-right">{currency(profit)}</td>
                </tr>
                <tr>
                  <td className="text-right">Freight:</td>
                  <td className="text-right">{currency(prices.freight)}</td>
                </tr>
                <tr className="summary-row" style={{ backgroundColor: '#e0e0e0' }}>
                  <td className="text-right bold" style={{ fontSize: '16px' }}>GRAND TOTAL:</td>
                  <td className="text-right bold" style={{ fontSize: '16px' }}>{currency(grand)}</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <div className="grid gap-2 mb-3">
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  name="deduct"
                  checked={deductChoice === 'width'}
                  onChange={() => setDeductChoice('width')}
                />
                <span>Use Box 1 <strong>Width × Height</strong> (current: {feetInToText(box1.wFt, box1.wIn)} × {feetInToText(box1.hFt, box1.hIn)})</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  name="deduct"
                  checked={deductChoice === 'length'}
                  onChange={() => setDeductChoice('length')}
                />
                <span>Use Box 1 <strong>Length × Height</strong> (current: {feetInToText(box1.lFt, box1.lIn)} × {feetInToText(box1.hFt, box1.hIn)})</span>
              </label>
            </div>

            <div className="flex justify-end gap-2">
              <button
                className="border rounded px-3 py-1 bg-gray-100 hover:bg-red-300 hover:ring-4 ring-red-500 focus-visible:ring-4 focus-visible:ring-red-500 transition" 
                onClick={skipDeduct}
              >
                Skip
              </button>
              <button
                ref={modalApplyBtnRef}
                className="border rounded px-3 py-1 bg-black text-white hover:bg-gray-700 hover:ring-4 ring-white transition focus-visible:ring-4 focus-visible:ring-white" 
                onClick={applyAutoDeduct}
              >
                Apply Deduction
              </button>
            </div>
